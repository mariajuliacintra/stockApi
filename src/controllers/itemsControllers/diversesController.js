...
        const query = 'INSERT INTO diverses (name, aliases, brand, description, technicalSpecs, quantity, expirationDate, batchNumber, fkIdLocation) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
            if (quantity > 0) {
                await TransactionController.createTransaction(fkIdUser, 'diverses', itemId, 'IN', quantity, 0, quantity);
            }
...
        
        try {
            const oldDiversesResult = await queryAsync('SELECT quantity FROM diverses WHERE idDiverses = ?', [idDiverses]);
            if (oldDiversesResult.length === 0) {
                return res.status(404).json({ message: 'Item diverso não encontrado para atualização.' });
            }
...
            
            const query = 'UPDATE diverses SET name = ?, aliases = ?, brand = ?, description = ?, technicalSpecs = ?, quantity = ?, expirationDate = ?, batchNumber = ?, fkIdLocation = ? WHERE idDiverses = ?';
            const values = [name, aliases, brand, description, technicalSpecs, quantity, expirationDate, batchNumber, fkIdLocation, idDiverses];
            
