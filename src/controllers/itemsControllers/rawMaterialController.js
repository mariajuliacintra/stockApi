...
            if (quantity > 0) {
                await TransactionController.createTransaction(fkIdUser, 'rawMaterial', itemId, 'IN', quantity, 0, quantity);
            }
...
        
        try {
            const oldRawMaterialResult = await queryAsync('SELECT quantity FROM rawMaterial WHERE idRawMaterial = ?', [idRawMaterial]);
            if (oldRawMaterialResult.length === 0) {
                return res.status(404).json({ message: 'Matéria-prima não encontrada para atualização.' });
            }
...
            
            const query = 'UPDATE rawMaterial SET name = ?, aliases = ?, brand = ?, description = ?, technicalSpecs = ?, quantity = ?, batchNumber = ?, fkIdLocation = ? WHERE idRawMaterial = ?';
            const values = [name, aliases, brand, description, technicalSpecs, quantity, batchNumber, fkIdLocation, idRawMaterial];
            
