...
module.exports = class ProductController {
  ...
  static async searchProductByName(req, res) {
    ...
    const query = "SELECT * FROM product WHERE LOWER(name) LIKE LOWER(?)";
    const values = [`${name}%`];
  ...
  static async updateProduct(req, res) {
    ...
    try {
      const oldProductResult = await queryAsync(
      ...
      const query =
        "UPDATE product SET name = ?, aliases = ?, brand = ?, description = ?, technicalSpecs = ?, quantity = ?, expirationDate = ?, batchNumber = ?, fkIdLocation = ? WHERE idProduct = ?";
      const values = [
        name,
        aliases,
        brand,
