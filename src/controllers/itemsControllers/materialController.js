...
module.exports = class MaterialController {
  ...
  static async searchMaterialByName(req, res) {
    const { name } = req.query;
    const query = "SELECT * FROM material WHERE LOWER(name) LIKE LOWER(?)";
    const values = [`${name}%`];
  ...
  static async updateMaterial(req, res) {
    ...
    try {
      const oldMaterialResult = await queryAsync(
        "SELECT quantity FROM material WHERE idMaterial = ?",
        [idMaterial]
      );
      if (oldMaterialResult.length === 0) {
        return res
          .status(404)
          .json({ message: "Material não encontrado para atualização." });
      }
      ...
      const query =
        "UPDATE material SET name = ?, aliases = ?, brand = ?, description = ?, technicalSpecs = ?, quantity = ?, expirationDate = ?, batchNumber = ?, fkIdLocation = ? WHERE idMaterial = ?";
      const values = [
        name,
        aliases,
        brand,
