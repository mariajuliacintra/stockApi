...
module.exports = class EquipmentController {
  ...
  static async searchEquipmentByName(req, res) {
    const { name } = req.query;
    const query = "SELECT * FROM equipment WHERE LOWER(name) LIKE LOWER(?)";
    const values = [`${name}%`];
  ...
  static async updateEquipment(req, res) {
    ...
    try {
      const oldEquipmentResult = await queryAsync(
        "SELECT quantity FROM equipment WHERE idEquipment = ?",
        [idEquipment]
      );
      ...
      const query =
        "UPDATE equipment SET name = ?, aliases = ?, brand = ?, description = ?, technicalSpecs = ?, quantity = ?, batchNumber = ?, fkIdLocation = ? WHERE idEquipment = ?";
      const values = [
        name,
        aliases,
        brand,
